...
function PhoneCodeVefify({ onCloseClick }) {
    ...
    const sendOTP = async () => {
        ...
        const formattedNumber = formatPhoneNumber(user?.countryCode, user?.phoneNumber);
        console.log('Formatted number:', formattedNumber);

        try {
            const response = await axios.post(
                `${import.meta.env.VITE_BACKEND_URL}/server/sms/send-verification`,
                { phone: formattedNumber, userId: user?._id },
...
export default PhoneCodeVefify