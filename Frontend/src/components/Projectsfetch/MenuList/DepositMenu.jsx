...
function DepositMenu({ isClose, handleOpenDepositMenu }) {
    ...
    const handleAddFunds = async () => {
        ...
        const formattedAmount = numericAmount.toFixed(2); // Ensures 2 decimal places

        localStorage.setItem('tempUserId', user?._id);
        localStorage.setItem('shouldOpenDepositMenu', 'true'); // Flag to reopen dialog


        try {
            const response = await axios.post(
                `${import.meta.env.VITE_BACKEND_URL}/server/payment/create-payment`,
                {
                    currency: 'USD',
                    userId: user?._id,
    ...
    useEffect(() => {
        ...
        const executePayment = async () => {
            ...
            if (paymentId && payerId) {
                ...
                try {
                    await axios.post(
                        `${import.meta.env.VITE_BACKEND_URL}/server/payment/execute-payment`,
                        {
                            paymentId,
                            payerId,
                            userId: storedUserId,
    ...
    const handleBlur = () => {
        ...
        if (!isNaN(numericValue)) {
            ...
            const formattedDisplay = numericValue.toLocaleString('en-US', {
    ...
    const handleAmountChange = (e) => {
        ...
        let processedValue = whole || '';
        if (decimal !== undefined) {
            processedValue += `.${decimal.substring(0, 2)}`;
        }
        ...
        if (!isNaN(num)) {
            setDisplayAmount(num.toLocaleString('en-US', {
                minimumFractionDigits: decimal ? Math.min(decimal.length, 2) : 0,
                maximumFractionDigits: 2
            }));
        } else {
            setDisplayAmount('');
                        ...
                        <>
                            ...
                            >
                                <div className="Depoti">
                                    ...
                                    >
                                        {t('Specify the amount you would like to deposit. (USD)')}
                                    </Typography>
                                </div>
                                {errorMessage && (
                                    <Typography
